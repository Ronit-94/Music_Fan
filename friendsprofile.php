    ...
    $(document).ready(function(){
       ...
       // alert(friend_privacy);
        ...
        for(var key in diaries){
            ...
            var html="";
            html += "<div class='main-container' id='post_"+diaries[key]['post_id']+"'><div class='name_title'><div class='title'><span><h3 style='color:#BC68C9'>"+diaries[key]['diary_title']+"</h3><div class='description'><h5 style= 'color:#BD2727;font-weight:bold; font-size:20px'>"+diaries[key]['description']+"</h5><h6 style='color:#ED2BC4; font-weight:bold' class='date'>"+diaries[key]['cr_date']+"</h6><h5>"+(diaries[key]['location']!==""?"<span style='color:blue' class='glyphicon glyphicon-map-marker'></span><a style='font-size:20px' href='http://localhost:81/location.php?loc="+diaries[key]['location']+"'<span class='location'>"+diaries[key]['location']+"</span></a>":"")+"</h5><div class='desc_comment'></span>"+(diaries[key]['multimedia']!==''?"<div  style='padding-left:100px;'><img src = '"+diaries[key]['multimedia']+"'width=200 height=200/></div>":"")+"</div><div class='likes_block' style='color:red'>Likes:<span id='like_block_"+diaries[key]['post_id']+"'></span><span class='like_symbol' id='like_symbol_"+diaries[key]['post_id']+"'></span></div><div id='comments_"+diaries[key]['post_id']+"' class='commments'><h6>----comments here----</h6></div></div></div>";
...
<img src="<?php echo $picture; ?>" width="300" height="300" /><br><br>
...
if($action==1)
{
    echo "<div style=font-weight:bold;color:#00FFE8>"."Message: <input type='text' name='message' placeholder='Message'> 
          <input type='submit' name='send_message' value='Send'>"."</div>"; 
}
?>                      
</form>
</div>
...
<?php
$query="select firstname,date_format(birthdate,'%D %M, %Y') as birthdate,email,country,lastname,country,fgenre,fsinger,fsong,picture from profile where user_id=$friend_id";
$result=mysqli_query($con,$query);
if(mysqli_num_rows($result)>0)
...
if($action==1)
{
    ...
    $result = mysqli_query($con,$query);
    if (mysqli_num_rows($result) > 0) {
    	echo "<h3 style=text-decoration:underline;color:#00FFE8><strong> Friends </strong>  </h3> ";
        // output data of each row
        while($row = mysqli_fetch_assoc($result)) {
            //echo $row["firstname"]. " ". $row["lastname"]."<br>";
            echo "<a style=font-weight:bold;color:#FA2F76;font-size:18px href='friendsprofile.php?id=".$row["user_id"]."' >".$row["firstname"]. " ". $row["lastname"]."</a><br>";
        }
    } else {
        echo "0 results";
    }
